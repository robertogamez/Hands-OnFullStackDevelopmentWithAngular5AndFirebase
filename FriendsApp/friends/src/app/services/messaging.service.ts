...
export class MessagingService {
    ...
    freshlyCreateChatIDEntry(uid: string, friendUid: string): string {
        ...
        this.fireDb.object(`${USER_DETAILS_CHILD}/${CHAT_MESSAGES_CHILD}/${uid}/${friendUid}`).set({
            key: key
        });

        this.fireDb.object(`${USER_DETAILS_CHILD}/${CHAT_MESSAGES_CHILD}/${friendUid}/${uid}`).set({
            key: key
        });
    ...
    createNewMessage(newMessage: Message) {
        ...
        this.fireDb.object(`${MESSAGE_DETAILS_CHILD}/${this.key}/${messageKey}`)
